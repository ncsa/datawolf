...
public class HPCExecutor extends RemoteExecutor {
	...
	@Override
	public State submitRemoteJob(File cwd) throws AbortException,
			FailedException {
		...
		try {
			t.start();
			WorkflowStep step = SpringData.getBean(WorkflowStepDAO.class)
					.findOne(getStepId());
			Execution execution = SpringData.getBean(ExecutionDAO.class)
					.findOne(getExecutionId());
			HPCToolImplementation impl = new ObjectMapper()
					.readValue(step.getTool().getImplementation(),
							HPCToolImplementation.class);
			...
			for (CommandLineOption option : impl.getCommandLineOptions()) {
				...
				case PARAMETER:
					String value = null;
					if (execution.hasParameter(step.getParameters().get(
							option.getOptionId()))) {
						value = execution.getParameter(step.getParameters()
								.get(option.getOptionId()));
					} else {
						value = step.getTool()
								.getParameter(option.getOptionId()).getValue();
					}
					if (option.isCommandline()) {
						...
						WorkflowToolParameter param = step.getTool()
								.getParameter(option.getOptionId());
				...
				case DATA:
					...
					if (filename == null) {
						try {
							filename = File.createTempFile("ci", ".tmp", cwd)
									.getAbsolutePath();
						} catch (IOException exc) {
							throw (new FailedException(
									"Could not create temp file.", exc));
						}
					}
					stagedFiles = cwd.getAbsolutePath()
							+ NonNLSConstants.PATH_SEP + filename;
					if (option.getInputOutput() != InputOutput.OUTPUT) {
						...
						Dataset ds = SpringData.getBean(DatasetDAO.class)
								.findOne(execution.getDataset(key));
						if (ds == null) {
							throw (new AbortException("Dataset is missing."));
						}
						try {
							InputStream is = SpringData.getFileStorage()
									.readFile(ds.getFileDescriptors().get(0));
			...
			try {
				session = SshUtils.maybeGetSession(new URI(contactURI),
						targetUser, home);
			...
			try {
				jc = JAXBContext
						.newInstance(new Class[] {
								edu.illinois.ncsa.gondola.types.submission.JobSubmissionType.class,
								edu.illinois.ncsa.gondola.types.submission.JobStatusListType.class,
								edu.illinois.ncsa.gondola.types.submission.JobStatusType.class,
								edu.illinois.ncsa.gondola.types.submission.ObjectFactory.class, });
			} catch (JAXBException e) {
				logger.error("Failed to instantiate gondola classes.", e);
				// e.printStackTrace(pw);
				throw new FailedException(
						"Failed to instantiate gondola classes.");
			}
			...
			File tmpScript = writeLocalScript(cwd, job.getScript(), stagingDir,
					normUuid, targetLineSep);

			// copy job script to target machine
			String scriptPath = stageFile(tmpScript, stagingDir, session,
			...
			throw (new FailedException(
					"Could not run transaction to get information about step.",
					e));
		} finally {
			try {
				if (t != null) {
				...
				throw (new FailedException(
						"Could not commit transaction to retrieve information about step.",
						e));
			}
		}
	...
	private String submit(JobSubmissionType job, String targetPath,
			SSHSession session, String lineSep)
			throws IllegalArgumentException, Exception, IOException {
		StringBuffer stdout = new StringBuffer();
		StringBuffer stderr = new StringBuffer();
		String[] lines = null;
		if (session != null) {
			String command = job.getSubmitPath() + NonNLSConstants.SP
					+ targetPath;
	...
	private JobSubmissionType createJob(File workflow, List<String> command,
		...
		try {
			element = (JAXBElement<?>) jc.createUnmarshaller().unmarshal(
					workflow);
		} catch (JAXBException e) {
	...
	private Map<String, String> getWorkflowInputs() throws FailedException {
		...
		try {
			...
			try {
				...
				logger.warn("A problem occurred closing fileLocations input.",
						e);
			}
		}
	}
	...
	protected void updateJobScript(ScriptType script,
		...
		while (iterator.hasNext()) {
			...
			content = content.concat(NonNLSConstants.SP + "-" + key
					+ NonNLSConstants.SP + fileLocation);
		}
	...
	private static String normalize(String uuid) {
		return uuid
				.replaceAll(NonNLSConstants.REMOTE_PATH_SEP,
						NonNLSConstants.DOT)
				.replaceAll(NonNLSConstants.BKESC, NonNLSConstants.DOT)
				.replaceAll(NonNLSConstants.REGDOT + NonNLSConstants.REGDOT,
						NonNLSConstants.ZEROSTR);
	}
	...
	public void cancelRemoteJob() {
		...
		if (session != null) {
			String command = job.getTerminatePath() + NonNLSConstants.SP
					+ jobId;
			try {
	...
	public State checkRemoteJob() throws FailedException {
		...
		try {
			if (session != null) {
				...
				lines = out.toString().trim()
						.split(NonNLSConstants.REMOTE_LINE_SEP);
			} else {
				List<String> args = new ArrayList<String>();
			...
			for (String line : lines) {
				...
				if (data != null) {
					if (data[0].equals(jobId)) {
						...
						} else if (jobState.equals(State.RUNNING)
								&& !storedJobInfo) {
							createJobInfo();
							return jobState;
						} else {
	...
	public State getGondolaLogFile() {
		...
		try {
			...
			BufferedReader stdoutReader = new BufferedReader(new FileReader(
					gondolaLog));
			boolean done = false;
			...
			try {
				...
				ByteArrayInputStream bais = new ByteArrayInputStream(stdout
				...
				t.commit();
				SpringData.getEventBus().fireEvent(
						new ObjectCreatedEvent(datasets));
				session.close();
				session = null;
			}
		} catch (Throwable e) {
			logger.error(
					"Error retrieving log file from remote system and writing it to a dataset.",
					e);
		}
		return State.FAILED;
	}
	...
	private void createJobInfo() {
		try {
			...
			logger.error(
					"Error getting log file from remote machine and storing as bean",
					e);
		}
	}
		...
		try {
			if (session != null) {
				...
				SshUtils.copyFrom(standardErr, stdErrLog.getAbsolutePath(),
						session);
				SshUtils.copyFrom(standardOut, stdOutLog.getAbsolutePath(),
						session);
			}
			// SshUtils.copyFrom(standardErr, stdErrLog.getAbsolutePath(),
			// session);
			...
			try {
				...
				ByteArrayInputStream bais = new ByteArrayInputStream(stdout
						.toString().getBytes("UTF-8"));
				FileDescriptor fd = SpringData.getFileStorage().storeFile(
						"stdout", bais);
				...
				bais = new ByteArrayInputStream(stderr.toString().getBytes(
						"UTF-8"));
